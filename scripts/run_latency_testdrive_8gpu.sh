...
echo "Latency settings: seq=${LATENCY_SEQ_LENGTHS:-64}, queries=${LATENCY_QUERIES:-5}, warmup=${LATENCY_WARMUP:-1}, max_new_tokens=${LATENCY_MAX_NEW_TOKENS:-32}"
...
for gpu in "${gpu_ids[@]}"; do
  ...
  (
    ...
    export LATENCY_QUERIES="${LATENCY_QUERIES:-5}"
