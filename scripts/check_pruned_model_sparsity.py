...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Count lm_head as a targeted linear weight when no report-level prune_lm_head metadata is present. "
            "Reports generated by the current pruning code infer this automatically."
        ),
    )
            ...
            if include_originals and model_info.get("model_path"):
                entries.append(
                    ModelEntry(
                        ...
                        include_lm_head=default_include_lm_head,
                    )
                )
            for method, method_info in (model_info.get("methods") or {}).items():
                ...
                entries.append(
                    ModelEntry(
                        ...
                        include_lm_head=bool(pruning.get("prune_lm_head", default_include_lm_head)),
                    )
                )
        return entries
...
def looks_like_linear_weight(name: str, ndim: int, include_lm_head: bool = False) -> bool:
    if not name.endswith(".weight") or ndim != 2:
        return False

    module_name = name[: -len(".weight")]
    if is_lm_head_weight(module_name) and not include_lm_head:
        return False
...
def inspect_model_dir(entry: ModelEntry, expected_sparsity: float, tolerance: float) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "include_lm_head": entry.include_lm_head,
    }
    ...
    try:
        for weight_file in weight_files:
            ...
            for name, tensor in state.items():
                ...
                if not looks_like_linear_weight(
                    name,
                    getattr(tensor, "ndim", 0),
                    include_lm_head=entry.include_lm_head,
                ):
                    continue
                module_name = name[: -len(".weight")]
    ...
    if args.report_json:
        entries.extend(
            entries_from_report(
                ...
                default_include_lm_head=args.include_lm_head,
