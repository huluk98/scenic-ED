...
This default launcher uses 5-epoch contrastive SFT plus 50% gradient one-shot pruning, matching the strongest 50% one-shot row from the current results. ONNX FP16 should use `CUDAExecutionProvider`. `ONNX_DISABLE_IO_BINDING=1` still uses CUDA EP but avoids Optimum's IO-binding call path during generation, which is where the recurring CUDA illegal-address Gather failure appears. `RUN_ACCURACY_SHARDED=1` splits benchmark accuracy examples across the listed GPUs and merges EM1/EM5 afterward, so the accuracy-retention check is much faster than one long single-GPU/CPU generation pass. `NUM_BEAMS=5 NUM_RETURN_SEQUENCES=5 MAX_NEW_TOKENS=128` is the comparable eval contract used for the earlier outputs. `SPLIT_EM1_EM5=1` scores EM1 from a separate top-1 beam call and EM5 from all 5 returned beams, avoiding ONNX beam-order issues where the correct answer is in the top 5 but not returned first. Using `NUM_BEAMS=1 NUM_RETURN_SEQUENCES=1 MAX_NEW_TOKENS=64` is faster but not comparable and can make apparent accuracy drop sharply. `ACCURACY_SHARD_STREAM_LOGS=1` streams each shard log to the console so the run does not look hung. `MAX_TRAIN_EXAMPLES=0` skips the full training-data generation pass by default; set `MAX_TRAIN_EXAMPLES=` only when you intentionally want full training EM. `ACCURACY_SHARD_PARALLELISM=4` uses the 8 GPU ids in safer waves of 4 workers; raise it to `8` only if the node is stable. `RUN_PYTORCH_ACCURACY=0` skips the slow PyTorch dense/pruned generation rows, and `RUN_RUNTIME_BENCHMARK=0` skips latency/TPS because those edge metrics are only a guideline. Dynamic ONNX INT8 may still be slower or fall back because CUDA EP does not accelerate every quantized operator.

To reuse an existing run that already finished training/pruning/export, pass its output directory and only rerun accuracy:

```bash
git pull && OUTPUT_ROOT=onnx_eval_outputs/clean_h20_contrastive_gradient50_YYYYMMDD_HHMMSS ACCURACY_ONLY=1 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 ACCURACY_GPU_IDS=0,1,2,3,4,5,6,7 ACCURACY_SHARD_PARALLELISM=8 ACCURACY_SHARD_STREAM_LOGS=1 MAX_TRAIN_EXAMPLES=0 NUM_BEAMS=5 NUM_RETURN_SEQUENCES=5 MAX_NEW_TOKENS=128 SPLIT_EM1_EM5=1 ONNX_DISABLE_IO_BINDING=1 FP16_ONNX_PROVIDER=CUDAExecutionProvider INT8_ONNX_PROVIDER=CUDAExecutionProvider bash scripts/run_clean_h20_onnx_fp16_int8.sh charent/ChatLM-mini-Chinese
```

If you only need the pruned-model accuracy and want to skip dense baselines too, add `RUN_ONNX_FP16_DENSE_ACCURACY=0 RUN_ONNX_INT8_DENSE_ACCURACY=0`.

For a quick smoke check only, add `FAST_ACCURACY=1`; do not compare that number to the 5-beam EM5 runs.
...
This trains contrastive SFT for 5 epochs on `data/SCENIC_full_anchor_positive_negative.json`, creates the 50% gradient one-shot pruned checkpoint, exports dense and pruned ONNX FP16, exports FP32 ONNX sources, dynamic-quantizes them to ONNX INT8, evaluates benchmark/training EM@1 and EM@5, then benchmarks isolated latency, p95 latency, TPS, peak memory, and model size. The launcher sets `ENFORCE_CONTRASTIVE_GRADIENT50=1` by default, so accidental overrides away from the highest-scored contrastive-SFT gradient-50 contract fail unless you intentionally disable the guard. Accuracy evaluations are fanned out across the listed GPUs, while latency is run sequentially so the timing numbers are not contaminated by parallel jobs. The final JSON is `<OUTPUT_ROOT>/all_deployment_em_latency_report.json` and includes `fine_tune`, `pruning_contract`, `onnx_artifacts`, `accuracy_delta_table`, `model_size_table`, `runtime_benchmark`, and target/whole-model sparsity from the pruning summary.
