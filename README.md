...
```bash
git pull && CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 NPROC_PER_NODE=8 ACCURACY_GPU_IDS=0,1,2,3,4,5,6,7 ACCURACY_SHARD_PARALLELISM=8 ACCURACY_SHARD_STREAM_LOGS=1 MAX_TRAIN_EXAMPLES=0 ONNX_DISABLE_IO_BINDING=1 ONNX_OPSET=18 FP16_ONNX_PROVIDER=CUDAExecutionProvider INT8_ONNX_PROVIDER=CUDAExecutionProvider bash scripts/run_clean_h20_onnx_fp16_int8.sh charent/ChatLM-mini-Chinese
```
